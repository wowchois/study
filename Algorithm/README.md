...
#### 플로이드 워셜 알고리즘
: 한 번 실행으로 모든 노드 간의 최단경로를 구할 수 있다. (음의 간선도 사용가능)   
시간 복잡도 = O(n^3) - 그래프 크기가 작은 경우에만 사용 가능!!  

ex) 문제 - level4_버스여행   

- 2차원 인접 행렬을 구성하고 노드-노드 이동 시, 중간 노드로 사용가능한 노드를 선택하고 더 짧은 길이를 선택해서 줄여나간다.  
1-3-4 이면 1->4로 가려면 중간노드 3으로 선택
