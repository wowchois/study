...
### unpacking
: * = (),{},[] 괄호를 한번 걷어내는 작업  
ex) zip(*[[1,0.0],[0.0.1],[0,1,0]])  
-> *으로 [1,0.0],[0.0.1],[0,1,0]만 남은 상태에서 zip을 하면 tuple로 첫번째요소끼리 묶이고, 두번쨰 요소끼리 묶임  
...
- cycle 발생되면 안된다.
- 크루스칼 : 간선 기준 트리 생성 (가중치가 작은 간선 먼저 선택)
- 프림 : 정점 기준 트리 생성
- 예제 문제 : 프로그래머스-섬연결하기
...
- 가장 적을때의 경우를 생각
- 반복되는 규칙을 찾기.
- 이전 값/규칙으로 현재 값/규칙을 만들 수 있는지 찾기.
